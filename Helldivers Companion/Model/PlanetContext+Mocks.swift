...
extension PlanetsDataModel {
    /// Returns a model backed by MockAPIService and pre-seeded with mock contexts
    /// so views that call `viewModel.context(for:)` work in Xcode Previews without a live fetch.
